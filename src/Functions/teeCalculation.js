...
const St1 = (i, j, state1) => {
  return (sizeA3(i, state1) + sizeB3(j, state1))*2*Lu(i)
}

const St2 = (i, j, state1) => {
  return (sizeA3(i, state1) + sizeB3(j, state1))*2*L2()
}

const teeSurface = (i, j, state1) => {
  return St1(i, j, state1) + St2(i, j, state1);
};

//Duct BOM

const bomPanel = (i, j, state1) => {
  let panelQ = teeSurface(i, j, state1);
  return panelQ;
};

const bomFlanges = (i, j) => {
  let flangesQ = sizeA(i) * 6 + sizeB(j) * 6;
  return flangesQ;
};
